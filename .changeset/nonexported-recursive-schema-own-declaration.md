...
The same fix a non-exported schema reached across files (its explicit annotation naming a type declared in another file, or imported from a file with no generated types of its own) still widens to `any` - that stopgap is unchanged.
