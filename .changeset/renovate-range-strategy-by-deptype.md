...
- `peerDependencies`: no longer overridden, so Renovate's built-in `"auto"` default applies, which always widens peer ranges (never narrows a floor). The dedicated `automerge: false` rule stays, since a peer range change still deserves human review even when it's a safe widen.
