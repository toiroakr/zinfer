...
export function formatAsDeclaration(
  ...
  // If unifyIfSame is enabled and types are identical
  if (unifyIfSame && result.input === result.output) {
    lines.push(
      `${schemaComment}export type ${typeName.unifiedName} = ${inputFormatted};`
    );
    return lines.join("\n");
  }
  ...
  if (!outputOnly) {
    lines.push(
      `${schemaComment}export type ${typeName.inputName} = ${inputFormatted};`
    );
  }
  ...
  if (!inputOnly) {
    ...
    lines.push(
      `${outputComment}export type ${typeName.outputName} = ${outputFormatted};`
    );
  }
