...
program
  .name("zinfer-mini")
  .description("Extract input/output types from zod/mini schemas")
...
program
  ...
  .option("--with-descriptions", "Include zod/mini .describe()/.meta() as TSDoc comments")
  .option("--generate-tests", "Generate vitest type equality tests alongside type files")
  .addOption(
    new Option(
      "--inline-type-references [scope]",
      'Inline a plain type an explicit z.ZodMiniType<T> annotation reaches, instead of referencing it: "project" (default when the flag is set) follows a reference within this project, "all" also follows one into a dependency package',
    ).choices(["project", "all"]),
  )
  .addOption(
    new Option(
...
// (e.g. `zinfer-mini --inline-type-references schema.ts`) would otherwise have
