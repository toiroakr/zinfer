...
/**
 * Extracts input and output types from zod/mini schemas using TypeScript Compiler API.
 */
export class ZodMiniTypeExtractor {
  ...
  private extractMultipleFromSourceFile(
    ...
    // Inject __Normalize and the type-utility import once for the main source file
    this.ensureNormalizeType(sourceFile);
    this.ensureTypeUtilityImport(sourceFile);
    ...
    for (const [localName, importInfo] of importedSchemas) {
      ...
      const isImportable =
        (importableFiles?.has(resolve(realpathSync(importInfo.sourceFilePath))) ?? false) &&
        (!context.generatedSchemaNames ||
      ...
      this.ensureTypeUtilityImport(importedSourceFile);
      try {
        ...
        this.cleanupTypeUtilityImport(importedSourceFile);
      }
    }
    ...
    // Clean up __Normalize and the type-utility import from the main source
    // file after all schemas are processed
    this.cleanupNormalizeType(sourceFile);
    this.cleanupTypeUtilityImport(sourceFile);
  ...
  /**
   * Injects a private `import type { input as ..., output as ... }` from
   * wherever this file reaches zod/mini, if not already present. Unlike
   * classic zinfer's `z.input<>`/`z.output<>` temp alias, this never relies on
   * the analyzed file having a `z`-named (or any) namespace binding in scope -
   ...
   * The __Normalize type and the type-utility import must already be present
   * (via ensureNormalizeType/ensureTypeUtilityImport).
   ...
   * Replaces patterns like <alias>.core.$InferInnerFunctionType<...> with
   * Function. The qualifying namespace alias varies with however the
   * analyzed file imports zod/mini, so only the stable `.core.$Infer...`
   * suffix is matched, not a hardcoded `z.` prefix.
  ...
  private simplifyZodFunctionTypes(typeStr: string): string {
    ...
    if (!typeStr.includes(".core.$Infer")) return typeStr;

    // Pattern prefixes for Zod internal function types
    const zodFunctionPrefixes = [
      /[\w$]*\.core\.\$InferInnerFunctionType</,
      /[\w$]*\.core\.\$InferOuterFunctionType</,
    ];
    ...
    while (modified) {
      ...
      for (const prefix of zodFunctionPrefixes) {
        const match = prefix.exec(result);
        if (!match) continue;
        const idx = match.index;

        // Find the matching closing bracket using bracket counting
        const startIdx = idx + match[0].length;
        let depth = 1;
        let endIdx = startIdx;
