...
export class DescriptionExtractor {
  ...
  /**
   * Gets the description from a zod/mini schema.
   *
   * zod/mini's `ZodMiniType` has no `.meta()`/`.describe()` instance method
   * (unlike classic zod) - `z.describe(text)`/`z.meta(obj)` (used inside
   * `.check(...)`) and `.register(z.globalRegistry, {...})` both store their
   * metadata out-of-band in zod's shared `globalRegistry` instead, so that is
   * read directly here. The `.meta()` / `_def.description` checks stay as
   * fallbacks in case a schema built with classic zod is mixed into the same
   * file (zod/mini and classic zod schemas can coexist).
   */
  private getDescription(schema: unknown): string | undefined {
    ...
    try {
      ...
      logDebugError("globalRegistry.get() call failed", error);
    }

    const zodSchema = schema as Record<string, unknown>;

    // Zod v4 (classic): use meta() method to get description
