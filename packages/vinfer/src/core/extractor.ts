...
export class ValibotTypeExtractor {
  ...
  private extractMultipleFromSourceFile(
    ...
    for (const [localName, importInfo] of importedSchemas) {
      ...
      const cacheKey = `${importInfo.sourceFilePath}:${importInfo.originalName}:${localName}:${isImportable}:${context.inlineTypeReferences ?? "off"}`;
   ...
   * (`import("valibot").Foo`) - treating it as a relative filename could
   * accidentally resolve to an unrelated same-named local file the caller
   * never intended to reach. A bare specifier is only resolved under
   * `scope === "all"`, and then only through TypeScript's own module
   * resolution (`ts.resolveModuleName`, from `containingFilePath` - the
   * file whose printed text is currently being expanded, not necessarily
   * the one that originally wrote the `import`, which the printed text
   * alone can't recover) rather than filesystem probing, for the same
   * reason: an ambient module with no backing file (e.g. a `declare module
  ...
  private resolveImportedSchemaType(
    ...
    const rawOutputType = this.resolveType(importedSourceFile, "__TempOutput", scope);
