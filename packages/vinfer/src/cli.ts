...
program
  ...
  .addOption(
    new Option(
      ...
      'Inline a plain type that an explicit v.GenericSchema<T> annotation reaches in another file, instead of referencing it. "project" (default) follows references within the project; "all" also follows one into a dependency package',
    ).choices(["project", "all"]),
  )
  .addOption(
    new Option(
