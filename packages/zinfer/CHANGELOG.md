...
- a201307: Fix a type reached only through an explicit type assertion (e.g. `.transform((x) => x as unknown as T)`) - not a `z.ZodType<T>` schema annotation - printing an unresolved bare identifier at a nested position, with no backing declaration or import in the generated file.
  ...
  Fixes a false-positive introduced by the "unresolvable computed key" check above: a single-element tuple (`[string]`) or an indexed-access type (`Pair[0]`) was mistaken for a `unique symbol` computed property key, discarding a perfectly safe expansion in favor of an unnecessary qualified import. Only a bracket immediately followed by `:`/`?:` (the checker's actual computed-property-key syntax) is treated as one now.
